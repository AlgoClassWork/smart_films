...
from PyQt5.QtCore import Qt # Импортируем Qt для выравнивания (пока не используется явно, но полезно)
...
window.setMinimumSize(900, 650) # Немного увеличил минимальный размер для лучшего вида
...
main_line.setContentsMargins(30, 30, 30, 30) # Увеличенные внешние отступы
...
app.setStyleSheet("""
    ...
    QTextEdit {
        ...
        box-shadow: 0 4px 10px rgba(0, 0, 0, 0.04); /* Очень деликатная тень */
    }
    ...
    QLineEdit {
        ...
        box-shadow: 0 2px 5px rgba(0, 0, 0, 0.02); /* Ещё более деликатная тень */
    }

    QListWidget {
        font-size: 15px; /* Чуть больший шрифт для элементов списка */
        box-shadow: 0 4px 10px rgba(0, 0, 0, 0.04); /* Такая же деликатная тень, как у QTextEdit */
    }
    ...
    /* Стиль для кнопки поиска (чтобы отличалась) */
    QPushButton#search_btn {
        background-color: #007bff; /* Классический синий для кнопки поиска */
        color: #ffffff; /* Белый текст */
        border: 1px solid #007bff; /* Рамка в цвет фона */
...
# Присваиваем objectName для кнопки поиска, чтобы стилизовать её по ID
search_btn.setObjectName("search_btn")


# --- Запуск приложения ---
window.show() # Отображаем главное окно
app.exec_() # Запускаем основной цикл обработки событий приложения
